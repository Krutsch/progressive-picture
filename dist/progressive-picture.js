...
function observe() {
  const observer = new IntersectionObserver((entries) => {
    for (const entry of entries) {
      if (entry.intersectionRatio > 0 && !progessiveLoaded.has(entry.target)) {
        ...
        preload(sources, "srcset", img).then((preloadedSource) => {
          ...
          preload([img], "src", img).then(
            (preloadedImage) =>
              preloadedImage && progessiveLoaded.add(entry.target)
          );
        });
      }
    }
...
async function forceLoad(img) {
  if (progessiveLoaded.has(img)) return; // Already preloaded
